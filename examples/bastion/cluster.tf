...
resource "kops_instance_group" "bastion-1" {
  cluster_name = kops_cluster.cluster.id
  name         = "bastion-1"
  role         = "Bastion"
  min_size     = 1
...
resource "kops_instance_group" "bastion-2" {
  cluster_name = kops_cluster.cluster.id
  name         = "bastion-2"
  role         = "Bastion"
  min_size     = 1
...
resource "kops_cluster_updater" "updater" {
  ...
  keepers = {
    ...
    bastion-1 = kops_instance_group.bastion-1.revision
    bastion-2 = kops_instance_group.bastion-2.revision
  }
